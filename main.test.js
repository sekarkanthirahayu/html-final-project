...
beforeEach(async () => {
  ...
  fetch = jest
    .fn()
    .mockReturnValueOnce(Promise.resolve({ json: () => dataJSON.books }));
...
describe('Testing the main tag in "Daftar Buku Perpustakaan" page', () => {
  ...
  describe('Test click delete button', () => {
    it('should be delete book', async () => {
      ...
      await waitFor(() => {
        const bookListAfterDelete =
          windowJSDOM.document.querySelectorAll('.book-item');
  ...
  describe('Test Edit Book Form', () => {
    it('should can update book', async () => {
      ...
      const buttonSubmit = windowJSDOM.document.querySelector(
        'input[type="submit"]'
      );
